...
function PncDashboard({ batches, visibleBatches, search, setSearch, showOnlyPnc, setShowOnlyPnc, setActiveNav }: { batches: Batch[]; visibleBatches: Batch[]; search: string; setSearch: (v: string) => void; showOnlyPnc: boolean; setShowOnlyPnc: (v: boolean) => void; setActiveNav: (v: string) => void }) {
  const pnc = visibleBatches.filter((batch) => batch.status === 'Bloqueado').filter((batch) => `${batch.id} ${batch.product}`.toLowerCase().includes(search.toLowerCase()))
  return <div className="space-y-6"><ModuleHeader eyebrow="Aislamiento preventivo" title="Cuarentena / Producto No Conforme" description="Control visual de lotes retenidos, desviaciones analíticas y disposición pendiente." icon={ShieldAlert} tone="red" /><div className="grid gap-4 sm:grid-cols-3"><MetricCard label="Lotes retenidos" value={String(batches.filter((b) => b.status === 'Bloqueado').length)} helper="requieren decisión" tone="red" /><MetricCard label="Envíos bloqueados" value="100%" helper="CEDI protegido" /><MetricCard label="Tiempo de respuesta" value="02:14 h" helper="promedio de turno" tone="amber" /></div><section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 p-5"><div><h3 className="font-bold text-slate-950">Cola de cuarentena</h3><p className="mt-1 text-sm text-slate-500">Lotes bloqueados registrados en el sistema.</p></div><div className="flex gap-2"><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar lote" className="h-9 rounded-lg border border-slate-200 px-3 text-sm outline-none focus:border-red-400" /><button onClick={() => setShowOnlyPnc(!showOnlyPnc)} className="rounded-lg border border-red-200 bg-red-50 px-3 text-xs font-bold text-red-700">{showOnlyPnc ? 'Mostrando PNC' : 'Filtrar PNC'}</button><button onClick={() => setActiveNav('Historial de lotes')} className="rounded-lg bg-slate-950 px-3 text-xs font-bold text-white">Historial</button></div></div><div className="divide-y divide-slate-100">{pnc.length ? pnc.map((batch) => <div key={batch.id} className="grid gap-3 px-5 py-4 md:grid-cols-[1fr_1.5fr_1fr_auto] md:items-center"><div><p className="font-mono text-sm font-bold text-slate-900">{batch.id}</p><p className="text-xs text-slate-500">{batch.date}</p></div><div><p className="text-sm font-semibold text-slate-800">{batch.product}</p><p className="text-xs text-red-600">{batch.deviation} · {batch.value}</p></div><div className="text-xs text-slate-500"><span className="font-semibold text-slate-700">Destino:</span> CEDI bloqueado</div><StatusPill status="Bloqueado" /></div>) : <div className="p-10 text-center text-sm text-slate-500">No hay lotes PNC para mostrar con este filtro.</div>}</div></section></div>
}

function HistoryDashboard({ batches, search, setSearch }: { batches: Batch[]; search: string; setSearch: (v: string) => void }) {
  const rows = batches.filter((b) => `${b.id} ${b.product}`.toLowerCase().includes(search.toLowerCase()))
  const approved = batches.filter((b) => b.status === 'Aprobado').length
  const blocked = batches.filter((b) => b.status === 'Bloqueado').length
...
function AutoScanModal({ isOpen, onClose, onComplete }: { isOpen: boolean; onClose: () => void; onComplete: (batch: Batch) => void }) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setProgress(0)
      setCurrentAction('')
      setGeneratedBatchId(`L-AUT-${Math.floor(1000 + Math.random() * 9000)}`)
    } else {
      setStep(0)
      setProgress(0)
      setCurrentAction('')
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="w-full max-w-xl overflow-hidden rounded-3xl border border-teal-500/30 bg-slate-900 text-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-slate-800 bg-slate-950/80 px-6 py-4">
          ...
          <button onClick={handleClose} className="rounded-lg p-2 text-slate-400 hover:bg-slate-800 hover:text-white">
        ...
        <div className="flex items-center justify-between border-t border-slate-800 bg-slate-950/80 px-6 py-4">
          ...
          <div className="flex gap-2">
            <button onClick={handleClose} className="rounded-xl border border-slate-700 px-4 py-2 text-xs font-bold text-slate-300 hover:bg-slate-800">
              Cerrar
            </button>
            {step === 5 && (
              <button
...
export default function QualityDashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#f4f7f8] text-slate-900">
      <AutoScanModal
        isOpen={globalScanOpen}
        onClose={() => setGlobalScanOpen(false)}
        onComplete={(batch) => handleSubmit(batch)}
      />
      ...
      <aside className={`fixed inset-y-0 left-0 z-30 w-64 border-r border-slate-200 bg-white transition-transform lg:translate-x-0 ${menuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <Link href="/" className="flex h-20 items-center gap-3 border-b border-slate-100 px-6 transition hover:bg-slate-50" title="Volver al inicio"><div className="flex size-9 items-center justify-center rounded-xl bg-teal-700 text-white"><FlaskConical className="size-5" /></div><div><p className="font-bold tracking-tight text-slate-950">AQUA<span className="text-teal-700">LAB</span></p><p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-slate-400">Control de calidad</p></div></Link>
      ...
      <div className="lg:pl-64">
        ...
        <main className="mx-auto max-w-[1500px] space-y-6 p-5 sm:p-8">
              ...
              <div className="space-y-2 max-w-2xl">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="flex items-center gap-1.5 rounded-full bg-emerald-500/20 px-3 py-1 text-xs font-bold text-emerald-400 border border-emerald-500/30">
                    <Zap className="size-3.5 text-emerald-400" /> Sensores IoT + IA Autónoma
                  </span>
                  <span className="text-xs font-semibold text-teal-300/80">Planta Norte · Envasado en Línea</span>
                </div>
                <h2 className="text-2xl font-extrabold tracking-tight text-white sm:text-3xl">
                  Inspección y Escaneo Automático de Calidad
                </h2>
                <p className="text-sm text-slate-300 leading-relaxed">
                  Sistema robótico de control de calidad: realiza la medición de pH, densidad y peso en cinta transportadora y emite el dictamen de <strong className="text-emerald-400 font-bold">Producto Conforme</strong> de forma 100% autónoma sin intervención de operadores.
                </p>
              </div>
              <button
                onClick={() => setGlobalScanOpen(true)}
                className="inline-flex items-center justify-center gap-3 rounded-2xl bg-gradient-to-r from-emerald-400 via-teal-300 to-emerald-400 px-6 py-4 text-base font-extrabold text-slate-950 shadow-xl shadow-emerald-500/25 transition hover:scale-105 active:scale-95 shrink-0 border border-emerald-300"
              >
                <Sparkles className="size-6 text-slate-950 animate-bounce" />
          ...
          {(activeNav === 'Resumen' || activeNav === 'Captura de resultados') && <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
            <QualityForm onSubmit={handleSubmit} onOpenAutoScan={() => setGlobalScanOpen(true)} />
